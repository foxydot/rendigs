...
class pb_backupbuddy_core {
	 ...
	 *
	 */
	function periodic_cleanup( $backup_age_limit = 172800, $die_on_fail = true ) {
		...
		$this->trim_remote_send_stats( $backup_age_limit );
	 ...
	 *	@return		null
	 */
	public function trim_remote_send_stats( $max_age = 172800 ) {
	...
	public function get_database_size( $profile_id = 0 ) {
		...
		$result = mysql_query( "SHOW TABLE STATUS", $wpdb->dbh );
		while( $rs = mysql_fetch_array( $result ) ) {
			$excluded = true; // Default.
			
			// TABLE STATUS.
			$resultb = mysql_query( "CHECK TABLE `{$rs['Name']}`", $wpdb->dbh );
